...
0. 🧭 **เลือกโหมดจากคำสั่งของพี่ก่อน:**
   ...
   - **มีชื่อระบบ/โจทย์อยู่ในคำสั่ง** (เช่น "ช่วยประเมินราคา **ระบบคูปอง** ให้หน่อย") → **Inline mode**
     ...
     - สร้าง `input/<slug>/notes.md` บันทึกโจทย์ไว้ก่อน (พี่ไม่ต้องวางไฟล์เอง) ·
       `<slug>` = ชื่อระบบแบบ kebab-case อังกฤษ (เช่น `coupon-system`) · ใช้ `source = "input/<slug>"`
...
2. เลือก **1 โปรเจกต์** ที่ยังไม่ทำ → อ่านเอกสารต้นทางในโฟลเดอร์นั้น
   ...
   > "เอกสารต้นทาง" = ไฟล์ใน `input/` (folder) หรือข้อความในคำสั่ง (inline) · ลองแตก requirement เป็น module/scope ในใจก่อน
   > - **แตก module ได้อย่างมั่นใจ** → เดินหน้าทำจนจบเลย (ช่องว่างเล็ก ๆ บันทึกเป็น assumption) — ไม่ต้องถามพี่
   > - **บางจนแตก module ไม่ได้ถ้าไม่เดาเยอะ** (โจทย์ลอย ๆ เช่น "อยากได้ระบบคิว" / "ระบบคูปอง") → **หยุด ถามพี่ก่อน**
   >   วิธีถาม = **เสนอ scope/ฟีเจอร์ที่ฉันเดาให้พี่ยืนยัน/แก้** (ใช้คำถามแบบเลือกได้ ไม่ให้พี่พิมพ์เองทั้งหมด) แล้วค่อยทำต่อ
   >   · **Inline mode:** พอพี่ยืนยัน scope แล้ว ให้ **append scope ที่ตกลงลง `input/<slug>/notes.md`** ด้วย (ร่องรอยครบ)
